...
public class DijkstraAlgorithm extends ShortestPathAlgorithm {
	...
	protected Label[] labels_init() {
		...
		// l'initialisation se fait automatiquement à partir du constructeur de la
		// classe label
	...
	public ShortestPathSolution doRun() {
		...
		// Cette boucle tourne tant que le sommet destination n'est pas marqué ET tant
		// que le tas n'est pas vide
		// çad
		// Cette boucle s'arrête quand le sommet destination est marqué OU quand le tas
		// est vide
		while (!label_destination.getMarque() && !tas.isEmpty()) {

			// On extrait l'élément du tas avec le coût minimal (1ère itération: label
			// origine)
			...
			// On crée une liste de successeurs du noeud courant pour simplifier leur
			// parcours
			...
			for (Arc arc_courant : successeurs) {
				...
				// Condition importante pour vérifier que l'arc en question est permis selon le
				// mode de déplacement
				if (data.isAllowed(arc_courant)) {
					...
					if (!destination_iter.getMarque()) {
						...
						if (destination_iter.getCost() > label_courant.getCost() + data.getCost(arc_courant)) {
							...
							// si le noeud destination de l'arc est déjà dans le tas, on l'enlève (pour
							// l'ajouter à la fin quand il sera màj)
							try {
							...
							// on met à jour son père qui devient le noeud courant (on met tout l'arc car
							// c'est plus pratique)
							...
							// System.out.println("Cout label courant: " +destination_iter.getTotalCost());
							// //pour A*
		...
		if ((label_destination.getPère() == null && (data.getOrigin().compareTo(data.getDestination()) != 0))
		...
		// Sinon, on a trouvé la destination
			...
			if (data.getOrigin().compareTo(data.getDestination()) == 0) {
				...
				// On commence par la destination (puisqu'on s'y est arrêté) et on retrace le
				// chemin en ajoutant à chaque fois le père du noeud
				// jusqu'à arriver au noeud origine en mettant à jour à chaque fois le label
				// courant
